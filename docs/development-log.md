...
- 2026-07-26: Simplified the Line-up import flow to use the Timetable.lol catalogue only, removed empty festival-image placeholders from upcoming cards, changed Add Festival from a modal to a dedicated page, made modal surfaces solid rather than using the custom background image, and refined the settings close animation to collapse from the bottom upward.
- 2026-07-26: Added a subtle 180ms vertical lift-and-fade transition for ordinary Ionic route changes. Settings is deliberately excluded so its bespoke red expansion and retraction animation remains intact.
- 2026-07-30: Reworked festival addition around the Timetable.lol catalogue, including two-character search suggestions, a browse page, selection confirmation, duplicate prevention, and atomic festival plus line-up creation. Kept custom festival creation as a secondary route, made published line-ups read-only apart from must-see choices, and documented location metadata as future work.
- 2026-07-30: Audited the future-work and release notes against the shipped app. Kept accounts/cloud sync as a post-v1 consideration and removed it as a pre-release requirement; clarified that GitHub security tooling is already enabled and that release work is ongoing review rather than setup.
- 2026-07-30: Added a reviewed, bundled Timetable.lol location catalogue. Catalogue additions now carry a display location and city-level coordinates without live geocoding; existing imported festivals are backfilled safely when the app loads. Location review is now part of the catalogue-refresh checklist.
- 2026-07-30: Made the Browse-to-Add selection resilient to route query changes and exposed the reviewed location in the confirmation card. Recorded high-priority automatic location enrichment for new catalogue events and deferred festival-style filters.
- 2026-08-13: Replaced the legacy Timetable.lol artist-act-index refresh source with the public Events and event-specific planner-data API. The bundled, offline-first catalogue now includes only live events with importable timed sets, API venue/address metadata, provider artwork, and stored ticket/resale metadata. Untimed or malformed rows and unsupported live events are reported during refresh rather than imported silently.
- 2026-08-13: Refined Dependabot grouping so Angular platform and non-Angular routine updates are grouped independently for both version and security update paths.
- 2026-08-13: Added the supplied RR-road SVG as the shared Home-header and launch-screen logo while retaining the established native application icon. A future branding pass will explore a lighter or white logo treatment for stronger contrast on red surfaces.
